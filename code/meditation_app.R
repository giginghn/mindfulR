...
tidy_all_nrc <- tidy_all %>%
  inner_join(nrc,by='word') %>%
  filter(!lexicon_nrc %in% c("positive", "negative"))
...
radar_chart1 <- radar_chart[c(7,2,6,8,4,10,3,1,9,5),c(1,2,3)]
radar_chart2 <- radar_chart[c(7,2,6,8,4,10,3,1,9,5),c(1,4,5)]
radar_chart3 <- radar_chart[c(7,2,6,8,4,10,3,1,9,5),c(1,5,6)]
...
chartJSRadar(radar_chart1, polyAlpha = 0.1, lineAlpha = 0.8, maxScale = 30,
             colMatrix = color1, byrow = F, nrow = 3)


chartJSRadar(radar_chart2, polyAlpha = 0.1, lineAlpha = 0.8, maxScale = 40,
             colMatrix = color2, byrow = F, nrow = 3)


chartJSRadar(radar_chart3, polyAlpha = 0.1, lineAlpha = 0.8, maxScale = 40,
